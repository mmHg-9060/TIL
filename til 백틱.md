...
스코프: 변수가 살아 있는 유효 범위  
[지역 변수 스코프]  
가장 흔하게 접하게 되는 스코프로, 중괄호 안에서 선언된 변수들  
특징: 선언된 블록 안에서만 생존하다가, 블록이 끝나면 메모리에서 사라짐
...
public class Scope2_1 {
    public static void main(String[] args) {
        int x = 10; // x라는 변수가 새롭게 만들어짐
        if (x > 5) {
            int y = 20; // y라는 변수가 이 if문 안에서만 만들어짐
            System.out.println(x + y);
        } // y 사망
        System.out.println(y); // y를 사용할 수 있는 곳이었던 if문을 빠져나와버렸으므로 오류 발생
    }
} // x 사망
```
[스코프의 존재 이유]  
...
```
위 코드의 임시 변수 temp는 if 조건이 만족할 때 잠시 임시로 쓰이는 변수이다.  
그런데 임시 변수 temp가 main() 코드 블록에 선언되어 있는데, 이런 경우, 어떤 문제가 발생할까?  
아니면, 아무 문제도 안 생길수도 있지 않을까? 안타깝게도 메모리와 미래의 나와 팀원이 나를 저주하게 될 것이다.  
먼저, 비효율적인 메모리 사용으로 문제가 생긴다.  
이 temp는 if코드 블록에서만 필요하기 때문에, if코드 블록에서 선언되더라도 큰 문제 없이, 오히려 더 좋은 코드가 된다. 하지만, 이 temp는 main()이 끝날 때까지 같이 살아버리기 때문에, 메모리가 불필요하게 낭비된다.  
만약 정상적으로 if코드 블록 안에 temp를 선언했다면 자바를 구혀하는 곳에서 if코드 블록의 종료 시점에 이 변수를 메모리에서 제거하기 때문에, 좀 더 효율적으로 메모리를 사용할 수 있었을 것이다.  
그 다음으로는 코드 복잡성 증가가 있다.  
좋은 코드는 무엇일까? 에러가 안 나는 코드? 변수명이 명확한 코드? 아마도 다 맞을 것이다. 하지만 앞의 경우에는 모두 "코드에 쓰잘대기 없는 이상한게 없는, 다시 말해 군더더기 없는 코드" 가 전제되어야 하지 않을까?  
temp는 if코드 안에서만 필요하기 때문에, if코드 블록 안에 temp를 선언했다면, if코드 블록이 끝나더라도, temp를 전혀 생각 안 하고 작성해도 되고, 그 용량 몇 안 된다는 사람의 작업 기억에도 생각해야 할 변수를 하나 줄이므로써 도움이 될 것이다. 하지만, 지금은 if코드 블록이 끝나더라도 변수 temp가 살아있기 때문에, main이 끝날 때까지 신경을 써줘야 한다. "지금 코드 보니까 11줄, 12줄밖에 안 되는데, 그 잠깐 생각하는게 어렵냐?" 라고 할 순 있겠지만, 우린 취업을 목표로 하고, 그에 맞춰 실무에 맞는 코드 감각을 길러야 하는데, 과연 실무에서 20줄짜리 자바 코드 하나 던져놓고 "자, 이거 유지보수 잘 해주면 우리 회사에서 밥 벌어 먹고 살 수 있게 해 줄게" 라는 말을 해줄 회사가 있을까? 나 말고 전부 프로그래밍을 못 한다면 그럴 순 있겠지만 현실은 아니다.  
[형변환]
먼저 우리가 과거에 배웠던 Byte, int, long 이런걸 끌고 와보자면  
int보다 long이 더 컸었다.  
그렇다면, int 범위 안에 있는 80을 예로 들어보자.  
이 80은 int에도 넣을 수 있고, long에도 넣을 수 있을까?  
당연히 된다. long > int이므로, int에 들어갈 수 있다면, long에도 들어갈 수 있다.  
이번에는 순서를 바꿔보자.  
int의 범위가 대략 21억이니까, 22억(2,200,000,000)으로 잡아보자. long에 들어갈 수 있을까?  
들어갈 수 있다. long의 범위는 대략 -922경부터 922경까지이므로, 22억까지는 거뜬히 들어가고도 남는다.  
그렇다면 이제, int에는 22억이 들어갈 수 있을까?  
안타깝게도 int는 21억을 아득히 넘어버린 22억이라는 숫자를 못 받아들이고, 오버플로우를 내뱉어서 대략 -20억으로 저장해버리고 말 것이다.  
